(ns notespace-sicmutils-example.ex1-44
...
1.44](https://tgvaughan.github.io/sicm/chapter001.html#Exe_1-44) from
GJS's [Structure and Interpretation of Classical
Mechanics](https://tgvaughan.github.io/sicm/), using
the [SICMUtils](https://github.com/sicmutils/sicmutils) Clojure library and
the [Notespace](https://github.com/scicloj/notespace) rendering environment.

I'll add more explanation and text; but first, a demonstration that any of this
works at all."]
...
["Next, let's run the simulation and collect data on each coordinate's
evolution.
...
["Composing `e/Lagrangian->state-derivative` with `L-double-pendulum` produces a
state derivative that we can use with our ODE solver:"]
...
["Finally, two default parameters for our simulation. We'll record data in steps
of 0.01 seconds, and simulate to a horizon of 50 seconds."]
...
(The interface here could use some work: `e/integrate-state-derivative` tidies
...
(last raw-regular-data)

["## Measurements, Data Transformation"]

["Next we'll chart the measurements trapped in those sequences of state
tuples.

The exercise asks us to graph the energy of the system as a function of time.
Composing `e/Lagrangian->energy` with `L-double-pendulum` gives us a new
...
likes Clojure dictionaries; `transform-data` turns our raw data into a sequence
of dictionary with all values we might care to explore. This includes:
...
["The following forms transform the raw data for each initial condition and bind
the results to `chaotic-data` and `regular-data` for exploration."]
...
a [Vega-Lite](https://vega.github.io/vega-lite/) spec for visualizing the system.
...
sequence of `x, y` coordinates tagged with distinct IDs for each pendulum bob's
points:"]
...
graphs the position of each pendulum bob through time, with the actual pendulum
itself overlaid. Scroll down a bit to see it in action."]
...
With those tools in hand, let's make some charts. I'll call this first chart the
`system-inspector`; this function will return a chart that will let us evolve
the system with a time slider and monitor both angles, the energy error, and the
pendulum bob itself as it evolves through time."]
...
`angles-plot` generates a plot, with no animation, showing both theta angles and
...
(defn angles-plot [data]
  (hanami-common/xform
   ...
   :HCONCAT [(hanami-common/xform
              hanami-templates/vconcat-chart
              :VCONCAT [(k-vs-time data :theta1)
                        (k-vs-time data :theta2)])
             (hanami-common/xform
              hanami-templates/vconcat-chart
              :VCONCAT [(k-vs-time data :thetadot1)
